    ...
    <div class="relative z-10 w-full max-w-[1600px] mx-auto px-6 lg:px-12 pb-24">
        ...
        <div class="max-w-4xl">
            ...
            <div class="flex items-center gap-6 animate-reveal [animation-delay:0.3s]">
                ...
                <div class="text-3xl font-black text-accent-500 font-heading tracking-tight">${{ number_format($property->price) }}</div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="max-w-[1600px] mx-auto px-6 lg:px-12">
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
                <h3 class="text-xl font-bold text-gray-900 mb-6 font-heading">Contact Information</h3>
                <div class="space-y-4 mb-8">
                    <div class="flex items-center p-3 rounded-xl bg-gray-50">
                        ...
                        <span class="text-gray-900 font-semibold">{{ $property->user->name }}</span>
                    </div>
                    ...
                    <div class="flex items-center p-3 rounded-xl bg-gray-50">
                        ...
                        <span class="text-gray-900 font-semibold">{{ $property->user->phone }}</span>
                    </div>
                    @endif
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @if($property->status === 'available')
                        @auth
                        <a href="{{ route('properties.checkout', $property->slug) }}" 
                        ...
                        @else
                        <a href="{{ route('login', ['redirect' => route('properties.checkout', $property->slug)]) }}" 
                           class="w-full h-14 bg-accent-600 text-white font-black uppercase tracking-wider rounded-xl hover:bg-accent-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center group">
                            <span>{{ $property->type === 'sale' ? 'Purchase Now' : 'Rent Now' }}</span>
                            <svg class="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                        ...
                        @endauth
                    @else
                        <div class="w-full h-14 bg-gray-300 text-gray-600 font-black uppercase tracking-wider rounded-xl flex items-center justify-center cursor-not-allowed">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button class="h-12 bg-accent-600 text-white font-bold rounded-xl hover:bg-accent-500 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center">
                        ...
                        <button onclick="toggleSave('{{ $property->slug }}')" 
                                class="h-12 bg-accent-600 text-white font-bold rounded-xl hover:bg-accent-500 transition-all duration-300 flex items-center justify-center"
                        ...
                        @else
                        <a href="{{ route('login') }}" class="h-12 bg-accent-600 text-white font-bold rounded-xl hover:bg-accent-500 transition-all duration-300 flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                            </svg>
                            Save
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        ...
                        <span class="font-bold text-gray-900">{{ $property->category->name }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-50">
                        <span class="text-gray-500">Listed</span>
                        <span class="font-bold text-gray-900">{{ $property->created_at->format('M d, Y') }}</span>
                    </div>
